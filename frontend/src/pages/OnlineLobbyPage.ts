...
import { t } from '../i18n';
import { getBackendGameConfig } from '../utils/gameSettings';
        ...
        <div class="h-screen w-screen flex items-center justify-center bg-[#171A21] text-slate-100 p-4">
            <div class="bg-[#272A33] p-8 rounded-xl shadow-lg w-full max-w-md text-center">
                ...
                <div class="flex flex-col space-y-4">
                    ...
                    <button id="custom-game-button" class="w-full inline-flex items-center justify-center rounded-lg bg-yellow-600 hover:bg-yellow-500 text-white font-semibold py-3 px-5 transition" disabled>
                        ${t('custom_game_button')} (Soon)
                    </button>
                    <a href="/game-settings" data-link class="w-full inline-flex items-center justify-center rounded-lg bg-purple-600 hover:bg-purple-500 text-white font-semibold py-3 px-5 transition">
...
let customGameButtonHandler: (() => void) | null = null;
...
export function afterRender()
{
    ...
    document.getElementById('custom-game-button')?.addEventListener('click', customGameButtonHandler);
}
...
export function cleanup()
{
    ...
    if (customGameButtonHandler)
    {
        document.getElementById('custom-game-button')?.removeEventListener('click', customGameButtonHandler);
        customGameButtonHandler = null;
    }
}