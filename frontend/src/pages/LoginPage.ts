	...
	return `
		<div class="h-screen w-screen flex items-center justify-center bg-[#171A21] text-slate-100 p-4">
			<div class="bg-[#272A33] p-8 rounded-xl shadow-lg w-full max-w-md">
				<h2 class="text-2xl font-bold mb-6 text-center text-white">${t('login_title')}</h2>
				<form id="login-form" class="space-y-6">
					...
					<div class="flex flex-col sm:flex-row-reverse sm:items-center sm:justify-between gap-4">
						<button type="submit" class="w-full sm:w-auto inline-flex items-center justify-center rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white font-semibold py-3 px-6 transition">
							${t('login_button')}
						</button>
						<a href="/register" class="text-center sm:text-left font-medium text-indigo-400 hover:text-indigo-300 transition" data-link>
							${t('register_link')}
						</a>
					</div>
				</form>
